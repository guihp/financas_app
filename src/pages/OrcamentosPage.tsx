...
import { ShieldAlert, Plus, Pencil, Tag, AlertTriangle } from "lucide-react";
...
const OrcamentosPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Group totals spent
      const grouped = (txData || []).reduce((acc: Record<string, number>, tx) => {
  ...
  const handleSaveBudget = async () => {
    ...
    const val = parseFloat(budgetAmount.replace(",", "."));
              ...
              <CardContent className="p-5">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-2">
                    <span className="text-xl bg-muted p-1.5 rounded-md">{icon}</span>
                ...
                <div className="space-y-2">
                  ...
                  <Progress value={perc} className={`h-2.5 bg-muted ${barColor}`} />
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4 py-2">
            {!isEditMode && (
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">Escolha a Categoria</label>
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Limite (R$) Permitido para este mês</label>
              <Input type="number" placeholder="Ex: 800" value={budgetAmount} onChange={e => setBudgetAmount(e.target.value)} min="0" step="0.01" />
            </div>
          </div>
